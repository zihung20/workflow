...
`logic-workflow` is a TypeScript library for building typed, auditable workflow state machines. You describe a process as a graph of states and transitions, the engine executes it, and you persist the result as plain JSON.
...
`logic-workflow` catches all of these at compile time.
...
- **Not an orchestration server.** There is no hosted runtime, no queue, no scheduler. `logic-workflow` is a pure library — you provide the storage, the transport, and the trigger mechanism.
